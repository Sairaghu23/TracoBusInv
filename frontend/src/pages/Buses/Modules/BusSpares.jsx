...
<<<<<<< HEAD
    ...
    const toggleItem = (item) => {
        if (cart.find(c => c.item_id === item.item_id)) {
    ...
    const handleConfirmUsage = async () => {
        if (cart.length === 0) return alert("Select at least one product code.");
        if (!usageForm.new_reading || !usageForm.mechanic) return alert("Mechanic name and current odometer are required.");
        ...
        try {
            ...
            if (res.data?.status) {
                alert("Spare usage recorded successfully!");
                resetModal();
                fetchData();
            }
        } catch (err) {
            console.error(err);
            alert("Failed to record spare usage. Please try again.");
        }
    };
        ...
        <div className="max-w-[1440px] mx-auto p-6 space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-navy italic tracking-tighter uppercase">
                            {bus?.rc_plate_number || 'Vehicle'} <span className="text-slate-300 not-italic ml-2">/ Maintenance Hub</span>
                        </h1>
